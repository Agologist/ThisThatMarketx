...
import { Loader2, ChevronLeft, Trophy, Flag, Timer, Zap } from "lucide-react";
...
export default function RaceGame({ races, pollId: propPollId, optionAText, optionBText, option: propOption }: RaceGameProps = {}) {
  ...
  // Check if this is a special case challenge (25, 29, or 30)
  useEffect(() => {
    // Skip entirely for standalone mode
    if (isStandaloneMode) return;
    
    // Special handling for problematic challenges (25, 29, and 30)
    if (pollId === 25 || pollId === 29 || pollId === 30) {
      console.log(`Special handling for challenge ${pollId} - preventing restart`);
      // Mark as completed to prevent play
      setHasCompletedRace(true);
      setGameState("finished");
      setGameResult({ won: true, time: 30000 });
    }
  }, [pollId, isStandaloneMode]);
  
  // Auto-start game
  useEffect(() => {
    // Only auto-start when not in standalone mode, in "ready" state, and hasn't been completed
              ...
              <Card className="border-primary/30">
                <CardHeader>
                  <div className="flex justify-between items-center">
                    <CardTitle className="text-2xl">ThisThat.Market Battle</CardTitle>
